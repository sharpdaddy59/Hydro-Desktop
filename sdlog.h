// sdlog.h — append device readings to a CSV file on the microSD card.
//
...
// Append one CSV row per device every SD_LOG_INTERVAL_MS. Call from loop().
void sdlog_loop();
...
// True if `name` (a BARE filename — no path) is a safe hydro log file:
// no separators, no "..", whitelisted chars only, "hydro-" prefix,
