...
const char* device_hostname() {
  ...
  if (buf[0] == 0) {
    ...
    // and works before WiFi is initialized. WiFi.macAddress() returned
    // zeros when called from wifi_setup_begin (the very first caller),
    // because WiFiManager hasn't put the radio into any mode yet.
    //
    ...
    snprintf(buf, sizeof(buf), "hydro-dash-%02x%02x", byte4, byte5);
  }
  return buf;
}
