...
// SD_LOG_INTERVAL_DEFAULT_MIN), append one CSV row per device to a dated
...
// http_server_loop — so there is no separate task and no locking. Device
// readings are read from g_devices' atomics, exactly as the UI and the
// /devices handler do. The SD card owns the VSPI bus outright — the
// touchscreen that used to share it was removed in v0.1.14.
//
// Logging records; it never interprets. The CSV is a raw transcript of
// the same readings the screen and /devices already expose, with a time
// axis added — consistent with the dashboard's stateless stance (no
// code path ever reads the CSV back to make a decision).
...
static const char* CSV_HEADER =
  "timestamp,uptime_s,hostname,alias,kind,water_temp,air_temp,humidity,"
  "light,battery_pct,rssi,temp_unit,stale";
...
void sdlog_loop() {
  ...
  for (uint8_t i = 0; i < n; i++) {
    ...
    f.print(((DeviceKind)d.device_kind.load() == DeviceKind::BLE)
                ? "ble" : "http");
    f.print(',');
    csv_float(f, d.water_temp.load(), 1);  f.print(',');
    csv_float(f, d.air_temp.load(),   1);  f.print(',');
    csv_float(f, d.humidity.load(),   1);  f.print(',');
    csv_float(f, d.light.load(),      0);  f.print(',');
    { int bp = d.battery_pct.load(); if (bp >= 0) f.print(bp); }
    f.print(',');
    f.print(d.rssi.load());  f.print(',');
    f.print(temp_unit_suffix((TempUnit)d.temp_unit.load()));  f.print(',');
...
// Path-traversal gate: true only for a safe BARE hydro log filename.
...
bool sdlog_is_log_filename(const char* name) {
  ...
  if (strncmp(name, "hydro-", 6) != 0) return false;  // expected prefix
