...
// reboots into it.
//
// No PSRAM on the CYD, so unlike cores3-hydro there is no whole-image
// buffer — we stream. A failure at any chunk records s_error and aborts
// the Update; the running firmware is never touched until end() flips
// the boot pointer, so an interrupted upload is a safe no-op.
//
...
// referencing it here is what embeds it, so every hydro-dash .bin
...
static void handle_upload_chunk() {
  ...
  switch (up.status) {
    ...
    case UPLOAD_FILE_END:
      ...
      if (!s_fwid_found) {
        Update.abort();
        s_error = "not a hydro-dash image (identity marker missing)";
